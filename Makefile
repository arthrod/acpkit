...
check-pydantic-ai-matrix:
	@for version in $(PYDANTIC_AI_VERSIONS); do \
		...
		PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 uv run --package pydantic-acp --with pytest --with pytest-asyncio --with pyyaml --with openai --with mcp --with "pydantic-ai-slim==$$version" pytest -p pytest_asyncio.plugin tests/pydantic tests/test_native_pydantic_agent.py -q || exit $$?; \
		uv run --package pydantic-acp --with pytest --with pytest-asyncio --with pyyaml --with ty --with openai --with mcp --with "pydantic-ai-slim==$$version" ty check packages/adapters/pydantic-acp/src tests/pydantic examples/pydantic tests/test_native_pydantic_agent.py || exit $$?; \
	done
